...
| REC-5 | Recept ska kunna **aktiveras/inaktiveras** utan att raderas — via reglaget på receptkortet eller motsvarande val i kortets kontextmeny. Ett inaktivt recept visas nedtonat med grå accent. |
...
| TRD-9 | Varje diagram (Trender och Idag) visar alltid sitt faktiska datas lägsta och högsta värde som text under diagrammet (delad `MinMaxCaption`, regel 4) — oberoende av hur y-axelns rutnät råkar skala sig eller hur axeln avrundas till heltal (TRD-7). `IntervalBarChart` (TRD-8) ritar dessutom horisontella värdelinjer vid varje jämnt steg mellan lägsta och högsta axelvärde (delad `computeSmartYAxis`, regel 4) så mellanliggande värden går att avläsa utan att gissa. |
| TRD-10 | Trenders diagram stöder tvåfingerzoom och panorering horisontellt, samma känsla på samtliga sex diagram (#144) — linjediagrammen (`LineChartCanvas`) via Vicos inbyggda zoom-/scrollstöd, "Energi (dag)" (`IntervalBarChart`) via en egen handrullad gest-hantering eftersom komponenten inte bygger på Vico. Varje diagram är **helt utzoomat (hela den valda perioden synlig)** som standard, oavsett antal datapunkter (`LineChartCanvas` sätter `initialZoom = Zoom.Content`, i stället för Vicos standard som kan välja en mer inzoomad fast nivå för dataset med många punkter — uppföljning till #149). Zoom/panorering nollställs när **det diagrammets egna** period (TRD-3) byts — gäller korrekt för samtliga diagramtyper (#149: `LineChartCanvas` nycklade tidigare inte om sitt zoom-/panoreringstillstånd vid periodbyte). Gäller inte Idag-diagrammen (fasta 7-dagarsvyer utan periodval). |
| TRD-11 | Trender-ytan visar **Steg** och **Vilopuls** som egna diagram (Health Connect, samma källa som Idag-hälsokortet HEM-15/HLS-7), periodväljbara via sin egen periodväljare (TRD-3) — till skillnad från Idag-kortets sparklines som alltid visar senaste 7 dagarna. Kräver kopplad och behörig Health Connect-källa; tom period visar samma tomlägesmönster som övriga diagram (#146). |
