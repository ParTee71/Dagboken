...
| HEM-7 | Visa **sparkline-diagram** över **genomsnittlig energi per dag** senaste 7 dagarna, baserat på screenings (minst 2 datapunkter krävs, annars uppmaning att logga); länk till Trender-ytan (§17) för fördjupning. Delar linjestil (mjuk kurva + gradientfyllning, TRD-6) med Trender-diagrammet för visuell konsekvens. Diagrammet visar värden på **båda axlarna**: y-axel med energiskala, x-axel med veckodagsetiketter. Y-axeln skalas smart efter de faktiska värdenas min/max (TRD-7), inte alltid nollankrad. |
...
| HEM-15 | Idag visar ett **hälsokort** (Health Connect) med stegtrend för senaste 7 dagarna och vilopuls senaste veckan — se HLS-7 (§19). Döljs/ersätts av en diskret koppla-rad när data/behörighet saknas. Kortet visas längst ner på Idag, direkt ovanför energidiagrammet (HEM-7), eftersom båda är trender-över-tid-innehåll snarare än dagens checklistor. |
...
| TRD-2 | Flera serier — oavsett ursprung (aktivitet eller symptom) — kan väljas och overlagras i samma diagram, på en gemensamt beräknad skala (ingen separat y-axel per serie) som anpassas efter de valda seriernas min/max (se TRD-7) — inte alltid nollankrad. |
...
| TRD-7 | Diagrammets y-axel skalas smart efter de aktiva värdenas min/max (med marginal, avrundat till läsbara gränser) i stället för att alltid utgå från 0, så variationer i ett smalt värdeband blir läsbara. Gäller alla diagram i appen (delad `computeSmartYRange`, regel 4) — Trender (`LineChartCanvas`) och Idag-diagrammen (`SparklineChart`, se HEM-7/HLS-7). |
...
| HLS-7 | **Idag-skärmen** visar ett kompakt hälsokort med **stegtrend för senaste 7 dagarna** och **vilopulstrend för senaste 7 dagarna** (båda delad `SparklineChart`, regel 4, med värden på båda axlarna: mätvärde på y-axeln, veckodagsetiketter på x-axeln — visas när minst 2 dagar har ett värde; y-axeln skalas smart efter veckans faktiska min/max, TRD-7, inte alltid nollankrad), samt aktuell **vilopuls** senaste veckan som `StatPill`, när Health Connect är tillgängligt och behörighet beviljad. Vilopulsen (både StatPill och varje dag i trenden) tas i första hand från senaste `RestingHeartRateRecord`; saknas den (t.ex. Galaxy Watch via Samsung Health som inte skriver posten) **skattas** den i stället från periodens `HeartRateRecord`-prover (medelvärdet av den lägsta 5-percentilen ≈ den lägsta ihållande pulsen; ett enda artefaktlågt prov drar inte ner värdet) så att kortet visar en vilopuls i stället för "—". Saknas Health Connect eller behörighet visas en diskret "Koppla hälsa"-rad som djuplänkar till Hälsa-skärmen (§18) — **ingen** behörighetsbegäran sker på Idag. Kortet laddas fristående och blockerar inte Idag-renderingen. |
